...
export async function createDepartment(
  ...
  data: z.infer<typeof t.createDepartmentSchema>,
) {
  const existing = await prisma.department.findUnique({
    where: { organizationId_name: { organizationId, name: data.name } },
  });
  if (existing)
    throw new AppError('Department name already exists', HTTP.CONFLICT, ErrorCode.CONFLICT);
...
export async function updateDepartment(
  organizationId: number,
  id: number,
  data: z.infer<typeof t.updateDepartmentSchema>,
) {
...
export async function createCategory(
  organizationId: number,
  data: z.infer<typeof t.createCategorySchema>,
) {
  const existing = await prisma.assetCategory.findUnique({
    where: { organizationId_name: { organizationId, name: data.name } },
  });
  if (existing)
    throw new AppError('Category name already exists', HTTP.CONFLICT, ErrorCode.CONFLICT);

  return prisma.assetCategory.create({
    data: { ...data, extraFields: data.extraFields ?? undefined, organizationId },
  });
}
...
export async function updateCategory(
  organizationId: number,
  id: number,
  data: z.infer<typeof t.updateCategorySchema>,
) {
...
export async function promoteEmployee(organizationId: number, id: number, roleId: number) {
  ...
  if (!role || role.organizationId !== organizationId)
    throw new AppError('Invalid role', HTTP.BAD_REQUEST, ErrorCode.VALIDATION_ERROR);
...
export async function updateRole(
  organizationId: number,
  id: number,
  data: z.infer<typeof t.updateRoleSchema>,
) {
