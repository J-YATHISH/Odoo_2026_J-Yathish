...
export async function verifyAuditItem(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const data = await service.verifyAuditItem(req.user!.organizationId, parseInt(req.params.itemId as string, 10), req.user!.id, req.body);
    res.status(HTTP.OK).json(data);
  } catch (err) { next(err); }
}
...
export async function closeAuditCycle(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const data = await service.closeAuditCycle(req.user!.organizationId, parseInt(req.params.id as string, 10));
    res.status(HTTP.OK).json(data);
  } catch (err) { next(err); }
}
